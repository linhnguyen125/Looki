<template>
    <li class="dropdown" @click="markNotificationAsRead">
        <a
            ...
            aria-expanded="false"
        >
            <span class="glyphicon glyphicon-globe"></span> Notifcations
            <span class="badge">{{ totalUnreadNotification }}</span>
        </a>

        <ul class="dropdown-menu" role="menu">
            <li v-if="unreadNotification.length > 0">
                <notification-item
                    v-for="punread in unreadNotification"
                    v-bind:key="punread.data.user.id"
                    :unread="punread"
                ></notification-item>
            </li>
            <li v-else>
...
export default {
    ...
    methods: {
        ...
        showUserProfile(notify) {
            ...
            var notification = new Notification(
                notify.user.name + " is signup"
            );
            notification.onclick = function(event) {
                event.preventDefault(); // prevent the browser from focusing the Notification's tab
                window.open(
                    "http://blog-notify.test/admin/user/profile/" +
                        notify.user.id,
                    "_blank"
                );
            };
    ...
    mounted() {
        ...
        Echo.private("App.Admin." + this.userid).notification(notify => {
            console.log(notify);
            let newUnreadNotification = {
                data: { admin: notify.admin, user: notify.user }
            };
